...
function logoTag(): string {
  ...
  return `<img src="${src}" alt="Affy's" width="80" height="80" style="display:block;height:80px;width:auto;object-fit:contain;border:0;" />`;
}

function brandHeader(eyebrow: string): string {
  return `
    <div style="background:${ESPRESSO};padding:18px 24px;display:flex;align-items:center;gap:14px;">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
        <tr>
          <td style="vertical-align:middle;padding-right:14px;">${logoTag()}</td>
          <td style="vertical-align:middle;">
            <span style="color:${GOLD};font-size:11px;letter-spacing:0.18em;text-transform:uppercase;display:block;">${eyebrow}</span>
            <span style="color:#ffffff;font-size:14px;font-weight:600;font-family:Georgia,serif;">Affy&rsquo;s</span>
          </td>
        </tr>
      </table>
    </div>`;
